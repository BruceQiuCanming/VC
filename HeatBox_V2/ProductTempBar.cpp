...
/*void DrawTempFlat(CDC *dc,SWITCH_TEST_RESULT  *pSwitchTestResult ,SWITCH_CONFIG_PARA *pSwitchConfigPara ,CRect ChartRect,CTime BeginTestTime, bool IsOpenTemp)
{
	...
	CheckHeatOrCoolMode(*pSwitchConfigPara,&temp_range,BOX_DISC_FLAT);
...
#define PEN_WEIGHT	1	
	...
	//左边空1个FontSize.cx, 上面空 2 *FontSize.cy,	下面空 3 *FontSize.cy,
		...
		if(font_90d.CreateFontW(logFont.lfHeight,logFont.lfWidth,900,90,
		...
		{
			total[(int)x_min] += total[k];
		}
		for(int k = x_max; k < 200; k++)
		{
			total[(int)x_max] += total[k];
		}
		...
		for(int i = 0; i <=(int) x_gap; i++)
		{
			...
			CFont *oldFont_2 = MemDC.SelectObject(&font_90d);
			...
			{
				s.Format(_T("%dPcs,%3.1f%%"),total[(int)x_min + i],total[(int)x_min + i]*100.0f/sum_total);
				dt_rect.left   = rect.left;
				dt_rect.right  = rect.right;
				dt_rect.top    = rect.top -  FontSize.cy;
				dt_rect.bottom = rect.top;

				MemDC.TextOutW(dt_rect.left,dt_rect.top,s);
				//MemDC.DrawTextW(s, dt_rect,DT_LEFT);
				
			}
			MemDC.SelectObject(oldFont_2);
...
{
	int i;
	for(i =0 ; i < sizeof(m_SwitchTestResult) / sizeof(m_SwitchTestResult[0]); i++)
	{
...
//	DDX_Control(pDX, IDC_MSCHART1, m_Chart1);
	DDX_Check(pDX, IDC_CHECK_FONT_DIRECTION, m_bFontDirection);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CProductTempBar, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_FONT_DIRECTION,&OnBnClickedButtonFontDirection)
	ON_WM_LBUTTONDBLCLK()
END_MESSAGE_MAP()
...
//	this->m_Chart1.GetWindowRect(&ChartRect);
	...
	DrawTempFlat(m_BoxNr,dc,m_SwitchTestResult,&m_SwitchConfigPara, ChartRect,this->m_BeginTestTime,m_bOpenTemp,m_BoxType,m_bFontDirection); 
	this->ReleaseDC(dc);
...
void CProductTempBar::OnBnClickedButtonFontDirection()
{
	CDC *dc = this->GetDC();
	CRect ChartRect;
//	this->m_Chart1.GetWindowRect(&ChartRect);
