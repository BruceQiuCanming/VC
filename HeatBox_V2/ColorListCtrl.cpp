...
{
    m_columnSize = 8;//colSize;     // number of columns
...
void CColorListCtrl::SetColumnSize(int colSize)
{
...
//		return 4;
    ...
    for (int x=0; x<count; x++)
    {
		...
		lvc.pszText = (LPWSTR)(LPCTSTR)(phead->GetAt(x));
...
int CColorListCtrl::AddItem(CStringArray *line, int pos)
{
    ...
    {
        CListCtrl::SetItemText(count,x,line->GetAt(x));//(TCHAR*)LPCSTR(line->GetAt(x)) );
    }
    lvi.mask = LVIF_STATE;
    lvi.iSubItem = 1;
...
void CColorListCtrl::SetRowHeight(int nHeight) 
{
