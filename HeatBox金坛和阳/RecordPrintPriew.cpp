...
void CPrnPreview::DisplayPrnView()
{
	...
	if(dc == NULL)
	{
		...
		if(log_file.Open(theAppDirectory+"log.txt",CFile::modeReadWrite | CFile::modeCreate | CFile::modeNoTruncate))
		{
			log_file.SeekToEnd();
			log_file.Close();
		}
	...
	PrintReport(&MemDC,this->m_Title,m_BoxNr,m_BeginTestTime,m_EndTestTime,
		m_TestParaName,this->m_TestProductType,m_TestMemo, m_SwitchConfigPara,m_SwitchTestResult_Ex,m_bTestCloseLevel,250*m_PageNr,249+250*m_PageNr,m_HeatBoxType);
