...
//    int nHeight = SWITCH_PIC_HIGH;
	...
	for(i = m_ScrollBar_Switch_ID.GetScrollPos(); i < G_iMaxLedNr; i++)
	{
...
BOOL CTakeoutPage::OnInitDialog() 
{
	...
	m_ScrollBar_Switch_ID.SetScrollRange(0, G_iMaxLedNr - 1 , true);
...
{
	int i,sort_id = 0;
	SWITCH_TEST_RESULT_AND_LEVEL temp[500];
...
{
	source->level = CheckTestLevel(m_SwitchConfigPara,source->test_result,m_HeatBoxType);
	return;
...
//不合格的先选出来
		...
		switch(	CheckTestLevel(m_SwitchConfigPara,m_SwitchTestResult[i],m_HeatBoxType))
		{
...
//复大品
		case CLOSE_HIGH_LEVEL:
			if(m_bCloseHighLevel)
			{
					...
					CStringArray sArray;
					sArray.Add(str);
					m_ListTestResult.InsertItem(item_count,str);
...
//开门复位
		case OPEN_DOOR_RESET_LEVEL:
			if(m_bOpenDoorResetLevel)
			{
					...
					CStringArray sArray;
					sArray.Add(str);
					m_ListTestResult.SetItemText(item_count,1,level_str[OPEN_DOOR_RESET_LEVEL]);
					str.Format(_T("%5.1f"), m_SwitchTestResult[i].OpenTemp);
					sArray.Add(str);
					str.Format(_T("%5.1f"), m_SwitchTestResult[i].CloseTemp);
					sArray.Add(str);
...
//高规格
		case HIGH_LEVEL:
			if(m_bHighLevel && IsValidTemp(m_SwitchTestResult[i].OpenTemp))
			{
					...
					CStringArray sArray;
					sArray.Add(str);
					sArray.Add(level_str[HIGH_LEVEL]);
					str.Format(_T("%5.1f"), m_SwitchTestResult[i].OpenTemp);
					sArray.Add(str);
					str.Format(_T("%5.1f"), m_SwitchTestResult[i].CloseTemp);
					sArray.Add(str);
...
void CTakeoutPage::OnButtonTakeoutDlg() 
{
	...
	m_TakeoutDlg.ShowTestResult(false); 
	m_TakeoutDlg.m_WindowText = "离线拔板"; 
